...
export function AdminTemplateSection({ admin }: AdminTemplateSectionProps) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm dark:border-white/10 dark:bg-stone-950">
      <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 dark:border-white/10 xl:flex-row xl:items-center xl:justify-between">
...
function TemplateCard({
  ...
  const sourceClass =
    template.source === "ai" || template.source === "learned"
      ...
      : "border-stone-200 bg-stone-50 text-stone-600 dark:border-white/10 dark:bg-white/[0.04] dark:text-stone-300";
      ...
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <label className="flex cursor-pointer items-center gap-1.5 text-xs font-bold text-stone-500 dark:text-stone-400">
          <input
            ...
            className="h-3.5 w-3.5 rounded border-stone-300 text-emerald-600 focus:ring-emerald-500/50 dark:border-white/20 dark:bg-white/10"
          />
          上架
        </label>
      </div>
