  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center px-4 py-6">
      ...
      <section
        ...
        className="relative z-10 flex max-h-[88vh] w-full max-w-5xl animate-[fadeIn_0.2s_ease-out] flex-col overflow-hidden rounded-xl border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] shadow-xl"
      >
        ...
        {error ? (
          <div className="mx-5 mt-4 flex items-start gap-2 rounded-xl bg-[var(--theme-danger-soft)] px-3 py-2.5 text-sm font-semibold text-[var(--theme-danger-text)] ring-1 ring-[var(--theme-danger-border)] sm:mx-6">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 overflow-hidden md:grid-cols-[18rem_minmax(0,1fr)]">
          <aside className="min-h-0 border-b border-[var(--theme-border)] p-5 md:border-b-0 md:border-r sm:p-6">
            <div className="mb-4 flex items-center justify-between gap-2">
              <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--theme-text-muted)]">
                版本列表
              </span>
              <span className="rounded-lg bg-[var(--theme-surface-soft)] px-2 py-0.5 text-[11px] font-bold text-[var(--theme-text-muted)] shadow-sm">
                {revisions.length} 条
              </span>
            </div>
                ...
                revisions.map((revision) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedRevision?.id === revision.id
                        ? "border-[var(--theme-brand-border)] bg-[var(--theme-brand-soft)] text-[var(--theme-text-strong)] shadow-md ring-1 ring-[var(--theme-brand-border)]"
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="shrink-0 text-[11px] font-bold tabular-nums text-[var(--theme-text-muted)]">
                        {revision.wordCount.toLocaleString("zh-CN")} 字
                      </span>
                    </div>
                    <div className="mt-1.5 truncate text-xs font-bold text-[var(--theme-text-muted)]">
                      {revision.title || `第${revision.index}章`}
                    </div>
                    <div className="mt-2.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--theme-text-muted)]">
          ...
          <div className="flex min-h-0 flex-col p-5 sm:p-6">
            {loadingDetail ? (
              <div className="flex min-h-[22rem] items-center justify-center rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-surface-soft)] shadow-inner">
              ...
              <>
                <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
                  <div className="min-w-0">
                    <h4 className="truncate text-xl font-extrabold text-[var(--theme-text-strong)]">
                      {selectedRevision.title || `第${selectedRevision.index}章`}
                    </h4>
                    <p className="mt-1 text-xs font-bold uppercase tracking-wider text-[var(--theme-text-muted)]">
                  ...
                  <button
                    ...
                    className="theme-brand-gradient-bg inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-5 text-sm font-bold text-white shadow-md transition-all hover:brightness-105 hover:shadow-lg hover:shadow-[var(--theme-brand-500)]/25 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {restoring ? (
                ...
                {selectedSummary ? (
                  <div className="mb-4 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-surface-soft)] px-5 py-4 text-sm font-medium leading-relaxed text-[var(--theme-text-secondary)] shadow-inner">
                    <div className="mb-2 flex items-center gap-2 font-bold text-[var(--theme-text-strong)]">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-surface-soft)] px-6 py-6 shadow-inner">
                  {selectedRevision.content.trim() ? (
                    <div className="whitespace-pre-wrap break-words text-[15px] font-medium leading-8 text-[var(--theme-text-strong)]">
                      {selectedRevision.content}
                    </div>
                  ) : (
                    <div className="flex min-h-[16rem] items-center justify-center text-sm font-bold text-[var(--theme-text-muted)]">
                      这个历史版本没有正文内容。
                    </div>
                  )}
                </div>
              ...
              <div className="flex min-h-[22rem] items-center justify-center rounded-3xl border border-dashed border-[var(--theme-border)] bg-[var(--theme-surface-soft)] text-sm font-bold text-[var(--theme-text-muted)]">
                选择左侧历史版本后查看正文。
              </div>
            )}
          </div>
...
function LoadingState({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-center gap-3 py-8 text-sm font-bold text-[var(--theme-text-muted)]">
      <Loader2 className="h-4 w-4 animate-spin" />
      {label}
    </div>
  );
...
function EmptyState() {
  return (
    <div className="rounded-3xl border border-dashed border-[var(--theme-border)] bg-[var(--theme-surface-soft)] px-5 py-10 text-center">
      <History className="mx-auto mb-4 h-6 w-6 text-[var(--theme-text-muted)]" />
      <p className="text-sm font-bold text-[var(--theme-text-strong)]">还没有历史版本</p>
      <p className="mt-2 text-xs font-bold leading-relaxed text-[var(--theme-text-muted)]">
