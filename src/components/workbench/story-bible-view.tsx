...
export function StoryBibleView({
  ...
  return (
    <main className="app-work-surface relative min-h-dvh overflow-x-hidden pb-6 font-sans">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--theme-border)] bg-white/70 backdrop-blur-xl dark:bg-zinc-950/70">
        <div className="mx-auto flex max-w-[1480px] items-center gap-3 px-4 py-3 sm:px-6">
          <Link
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white text-zinc-600 shadow-sm ring-1 ring-[var(--theme-border)] transition hover:bg-zinc-50 hover:text-zinc-950 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800"
            title="返回作品"
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-950 text-white shadow-sm dark:bg-white dark:text-zinc-950">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              className="inline-flex h-10 items-center gap-2 rounded-xl bg-white px-3 text-xs font-bold text-zinc-600 shadow-sm ring-1 ring-[var(--theme-border)] transition hover:bg-zinc-50 hover:text-zinc-950 disabled:opacity-50 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800"
            >
              <RefreshCw className={cn("h-4 w-4", bible.loading && "animate-spin")} />
              刷新
            </button>
            <ThemeToggle className="h-10 w-10 rounded-xl bg-white shadow-sm ring-1 ring-[var(--theme-border)] dark:bg-zinc-900" />
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 mx-auto grid max-w-[1480px] gap-4 px-4 pt-4 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)_300px]">
        ...
        <section className="min-w-0 space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_120px_120px_auto]">
              <label className="relative block">
                ...
                <input
                  ...
                  className="h-10 w-full rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] pl-9 pr-3 text-sm font-semibold text-[var(--theme-text-primary)] outline-none transition focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
                />
              </label>
              <input
                ...
                placeholder="起始章"
                inputMode="numeric"
                className="h-10 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] px-3 text-sm font-semibold outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
              />
              <input
              ...
              <button
                ...
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-zinc-950 px-4 text-sm font-bold text-white shadow-sm transition hover:bg-black active:scale-[0.98] dark:bg-white dark:text-zinc-950"
              >
          ...
          {bible.notice ? (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200">
              {bible.notice}
            </div>
          ) : null}
            ...
            {bible.loading && !activeItems.length ? (
              ...
              <div className="flex min-h-[360px] flex-col items-center justify-center px-6 text-center">
                ...
                <button
                  type="button"
                  onClick={() => bible.openCreate(bible.activeSection)}
                  className="mt-4 inline-flex h-10 items-center gap-2 rounded-lg bg-zinc-950 px-4 text-sm font-bold text-white transition hover:bg-black dark:bg-white dark:text-zinc-950"
                >
                  <Plus className="h-4 w-4" />
                  新增条目
                </button>
          ...
          <section className="app-compact-panel p-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                inputMode="numeric"
                className="h-10 min-w-0 flex-1 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] px-3 text-sm font-semibold outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
              />
              <button
                type="button"
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-emerald-200 bg-emerald-50 text-emerald-900 shadow-sm dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-100"
    ...
    >
      ...
      <span className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-black text-[var(--theme-text-muted)] ring-1 ring-[var(--theme-border)] dark:bg-zinc-950/70">
        {count}
      </span>
    </button>
  );
      ...
      <div className="mb-2 flex items-start justify-between gap-3">
        ...
        <div className="flex gap-1 opacity-100 sm:opacity-0 sm:transition sm:group-hover:opacity-100">
          ...
          <button
            ...
            disabled={saving}
            title="删除"
            className="flex h-8 w-8 items-center justify-center rounded-md bg-red-50 text-red-600 ring-1 ring-red-200 transition hover:bg-red-100 disabled:opacity-50 dark:bg-red-500/10 dark:text-red-300 dark:ring-red-500/20"
          >
    ...
    <label className={cn("block", area && "sm:col-span-2")} key={key}>
      ...
      {area ? (
        <textarea
          ...
          rows={4}
          className="w-full resize-y rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] px-3 py-2 text-sm font-semibold leading-6 outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
        />
      ) : (
        <input
          value={draft[key]}
          onChange={(event) => updateDraft({ [key]: event.target.value })}
          className="h-10 w-full rounded-lg border border-[var(--theme-border)] bg-[var(--theme-surface-solid)] px-3 text-sm font-semibold outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
        />
      )}
    </label>
  );
