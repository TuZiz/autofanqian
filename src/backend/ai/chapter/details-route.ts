...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      await assertCanUseAiAction(user, AI_ACTIONS.chapterDetails);
    } catch (error) {
      return errorResponse(error);
    }
    ...
    if (!providers.length) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: `AI 未配置：当前“提取设定”使用 ${target.providerId}，但未检测到 ${envKey}。请在 web/.env 或 web/.env.local 配置后重启，或到后台“AI 模型配置”切换线路。`,
        },
        { status: 500 },
      );
    }
    ...
    const result = await runWithAiQuotaReservation(user, AI_ACTIONS.chapterDetails, () =>
      callAiText({
      providers,
    ...
    if (!result.ok || !result.text) {
      return NextResponse.json(
        {
          success: false,
          message: getReadableAiErrorMessage(result, getAiMetaCopy("details").errorFallback),
        },
        { status: 502 },
      );
    }
