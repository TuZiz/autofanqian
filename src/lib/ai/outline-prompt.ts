...
export function buildOutlineUserPrompt(input: OutlinePromptInput) {
  ...
  return [
    ...
    '  "totalChapters": 由你根据目标字数、平台节奏、故事体量估算出的长期目标总章数（数字，不要写字符串）,',
    '  "targetChapters": 与 totalChapters 相同的长期目标章数（数字）,',
    '  "plannedUntilChapter": 当前详细规划到第几章（默认 20-40，绝对不能超过 60）,',
    ...
    '      "endChapter": 卷结束章（数字）,',
    '      "detailLevel": "macro|detailed，首个可写窗口所在卷为 detailed，未来卷为 macro",',
    '      "status": "planned|locked，当前可写窗口为 planned，未来卷为 locked",',
    '      "desc": "本卷主线作用与情绪推进（60-180字）",',
    '      "segments": [',
    '        { "title": "小节名（2-12字）", "startChapter": 小节起始章（数字）, "endChapter": 小节结束章（数字）, "desc": "2-3句可执行剧情推进，不要空泛总结", "status": "planned" }',
    '      ]',
    '    }',
    "  ],",
    '  "characters": [',
    ...
    "",
    "要求：",
    "1) 不要一次性详细规划所有章节。只生成“全书宏观卷纲 + 首个可写窗口”。",
    "2) plannedUntilChapter 默认 20-40；如果故事开局需要更短，可 20 左右；绝对不能超过 60。",
    "3) 长期目标可以是 1560 章，但 segments 只能覆盖 plannedUntilChapter 以内的章节。未来卷只写宏观方向，不列出海量章节小节。",
    "4) 不要固定 400 章一卷，也不要机械平均分段；卷长按 AI 后续生成效率与剧情阶段动态决定。",
    "5) 卷数通常 2-10 卷，越长篇越偏宏观阶段；未来 locked/macro 卷可以没有 segments。",
    "6) 如果有目标字数，可按单章约 2500-4500 字估算总章数，但要优先服从平台节奏与剧情密度。",
    "7) characters 3-8 人，至少包含 protagonist、heroine、antagonist。",
    "8) 不要复刻仿书原作剧情；只抽象风格与结构。",
    "9) desc/segments[].desc 需要换行时，请在 JSON 字符串内使用 \\n 表示换行，不能输出未转义的真实换行。",
  ].join("\n");
}
